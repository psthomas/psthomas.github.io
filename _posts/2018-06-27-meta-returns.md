...
So if I assume a 95% confidence intervals unless otherwise stated, N=1000 samples from their simulations, and simulated samples that follow a normal distribution, I can back-calculate the standard deviation.  This assumes quite a lot (especially the N=1000 samples part), so I'd be open to advice on how to better handle this.  In addition to the calculations above, I filter out any GHCEA studies that were rated below 4.5 by reviewers on a 1-7 quality scale.  The end result is 653 interventions with standard deviations and cost effectiveness estimates.  
