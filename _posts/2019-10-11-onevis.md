...
If you want to find out what changed this election, just find the large bubbles with the long tails. And to find the crucial states in the electoral college, find the bubbles that just crossed over the y-axis. **Note that this visualization is interactive so you can zoom in for more detail, and filter categories using the legend**.
...
**Update**: Preliminary 2020 results have been added, based on aggregate county level results and the Elections Project state turnout data. The big story of 2020 so far seems to be turnout -- specifially turnout increases (and a slight shift left) in Low and Medium density places that was just enough to outweigh turnout increases in right leaning, Very Low Density places. It's also interesting to see a drop in turnout and Democratic margin in High Density places, **although all the votes haven't been counted yet so it's hard to know if this outcome is real**.

Catalist [won't be releasing](https://twitter.com/Catalist_US/status/1325905764691505152) their demographic data until January of 2021, so I'll add that when it becomes available.
